...
#Transitive closure table can only be created from Snapshot files
if [[ "${loadType}" == "ALL" || "${loadType}" == "SNAP" ]]
	then
	...
	echo "Inferred transitive closure requires Snapshot view"
fi
...
then
	...
	echo "Stated transitive closure requires Snapshot view"
fi
...
releaseDate=`ls -1 ${localExtract}/*${moduleStr}*.txt | head -1 | egrep -o '[0-9]{8}'`

#Determine the language and dialect from language Refset filename
language=`ls ${localExtract}/*Language* |grep ${moduleStr} |egrep -o '\-en(-[A-Z]{2})?'`	
...
addLoadScript der2_cRefset_LanguageTYPE${language}_MOD_DATE.txt langrefset
